...
@Service
class ServiceService(private val serviceRepository: ServiceRepository, private val harvestAdminClient: HarvestAdminClient) {
    private val logger = LoggerFactory.getLogger(ServiceService::class.java)
    private val mapper = jacksonObjectMapper()

    private fun ServiceEntity.toDTO(): ServiceDTO? = if (serviceType == ServiceType.SERVICE.name) {
        val values = mapper.convertValue<ServiceValues>(data)
        ServiceDTO(
            id = id,
            catalogId = catalogId,
            published = published,
    ...
    fun findServicesByCatalogId(catalogId: String): List<ServiceDTO> =
        serviceRepository.findByCatalogIdAndServiceType(catalogId, ServiceType.SERVICE.name)
            .mapNotNull { it.toDTO() }

    private fun findById(id: String, catalogId: String): ServiceEntity? = serviceRepository
        .findByIdOrNull(id)
        ?.takeIf { it.catalogId == catalogId && it.serviceType == ServiceType.SERVICE.name }

    fun findServiceById(id: String, catalogId: String): ServiceDTO? = findById(id, catalogId)?.toDTO()

    fun patchService(id: String, catalogId: String, operations: List<JsonPatchOperation>): ServiceDTO? = try {
    ...
    fun deleteService(id: String, catalogId: String) = try {
    ...
    fun createService(catalogId: String, values: ServiceValues): ServiceDTO = try {
        serviceRepository.save(
            ServiceEntity(
    ...
    fun publishService(id: String, catalogId: String): ServiceDTO? = try {
        findById(id, catalogId)
    ...
    fun unpublishService(id: String, catalogId: String): ServiceDTO? = try {
        findById(id, catalogId)
            ?.also { if (!it.published) throw CustomBadRequestException() }
            ?.let { dbo ->
                serviceRepository.save(
    ...
    fun publishedServicesInCatalog(catalogId: String): List<ServiceDTO> = serviceRepository.findByCatalogIdAndPublished(catalogId, true)
        .mapNotNull { it.toDTO() }
    ...
    fun getPublishedServiceInCatalog(id: String, catalogId: String): ServiceDTO = findServiceById(id, catalogId)
        ?.takeIf { it.published }
        ?: throw CustomNotFoundException()
}
