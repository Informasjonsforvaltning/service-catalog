...
@Service
class PublicServiceService(private val serviceRepository: ServiceRepository, private val harvestAdminClient: HarvestAdminClient) {
    private val logger = LoggerFactory.getLogger(PublicServiceService::class.java)
    private val mapper = jacksonObjectMapper()

    private fun ServiceEntity.toDTO(): PublicServiceDTO? = if (serviceType == ServiceType.PUBLIC_SERVICE.name) {
    ...
    fun findPublicServiceById(id: String, catalogId: String): PublicServiceDTO? = findById(id, catalogId)?.toDTO()

    fun patchPublicService(id: String, catalogId: String, operations: List<JsonPatchOperation>): PublicServiceDTO? = try {
    ...
    fun deletePublicService(id: String, catalogId: String) = try {
    ...
    fun createPublicService(catalogId: String, values: PublicServiceValues): PublicServiceDTO = try {
        serviceRepository.save(
            ServiceEntity(
    ...
    fun publishPublicService(id: String, catalogId: String): PublicServiceDTO? = try {
        findById(id, catalogId)
    ...
    fun unpublishPublicService(id: String, catalogId: String): PublicServiceDTO? = try {
        findById(id, catalogId)
            ?.also { if (!it.published) throw CustomBadRequestException() }
            ?.let { dbo ->
                serviceRepository.save(
    ...
    fun getPublishedPublicServiceInCatalog(id: String, catalogId: String): PublicServiceDTO = findPublicServiceById(id, catalogId)
        ?.takeIf { it.published }
        ?: throw CustomNotFoundException()
}
